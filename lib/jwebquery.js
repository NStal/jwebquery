...
jQuery.fn.downloadSrc = function(dir){
	...
	downloads.push(Download.createAndStart(target,dir,null,{page:page}));
    });
    return downloads;
}
var Download = function(url,dir,name,option){
    EventEmitter.call(this);
    this.url = url;
    this.dir = dir || "./";
    if(name && option.keepExtension){
	name+=path.extname(url);
    }
    this.name = name || path.basename(url);
    var option = option || {};
...
Download.createAndStart = function(url,dir,name,option){
    var _ = new Download(url,dir,name,option);
    this.queue.push(_);
    this.next();
    return _;
}
...
Page.create = function(url,option){
    ...
    option.delay = true;
    return this.delay(new Page(url,option));
}
...
Page.prototype.update = function(){
	...
	if(resp.headers["content-encoding"]=="gzip"){
	    zlib.gunzip(fixedBuffer,function(err,nb){
